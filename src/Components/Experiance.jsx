...
{EXPERIENCES.map((experiance,index)=>(
  <div key={index} className="mb-8 flex flex-wrap lg:justify-center">
    ...
    <p className="mb-2 text-sm text-neutral-400">{experiance.year}</p></motion.div>
    <motion.a
    whileInView={{opacity:1,x:0}}
    initial={{opacity:0,x:100}}
    transition={{duration:1.5}}
...
{experiance.technologies.map((tech,index)=>(<span key={index} className="mr-2 mt-4 rounded bg-neutral-900 px-2  py-1 font-medium text-purple-800 ">
{tech}
</span>))}
    </motion.a>
  </div>
